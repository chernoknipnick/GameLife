...
describe('порядок привычек', () => {
  function названия() {
    return Array.from(document.querySelectorAll('.today > .habits .habit__title')).map(
      (e) => e.textContent
    );
  }

  it('стрелки на ручке двигают привычку и сохраняют порядок', () => {
    /* Клавиатурный путь проверяется тестом, перетаскивание указателем —
       руками в браузере: jsdom не считает координаты, и проверка на нём
       мерила бы заглушку, а не поведение. */
    посадить({
      habits: [привычка({ id: 'a', title: 'Первая' }), привычка({ id: 'b', title: 'Вторая' })],
    });

    render(<App />);
    expect(названия()).toEqual(['Первая', 'Вторая']);

    fireEvent.keyDown(screen.getByRole('button', { name: /Переместить привычку «Первая»/ }), {
      key: 'ArrowDown',
    });

    expect(названия()).toEqual(['Вторая', 'Первая']);
    expect(JSON.parse(window.localStorage.getItem(STORAGE_KEY)).habits.map((h) => h.id)).toEqual([
      'b',
      'a',
    ]);
  });

  it('на краю списка стрелка ничего не ломает', () => {
    посадить({
      habits: [привычка({ id: 'a', title: 'Первая' }), привычка({ id: 'b', title: 'Вторая' })],
    });
  ...
  /* Перетаскивание указателем ищет карточку под пальцем по data-habit-id.
     Само движение jsdom не считает, но отсутствие этого признака в разметке
     оставило бы указательный путь мёртвым при зелёных тестах — так уже
     случилось однажды. */
  it('карточка помечена идентификатором, иначе тащить не над чем', () => {
    посадить({ habits: [привычка({ id: 'a', title: 'Первая' }), привычка({ id: 'b' })] });

    render(<App />);

    expect(screen.getByText('Первая').closest('[data-habit-id]').dataset.habitId).toBe('a');
  });

  it('у одинокой привычки ручки нет — тащить нечего', () => {
    посадить({ habits: [привычка({ title: 'Одна' })] });
